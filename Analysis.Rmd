...
Dist <- ggplot(Otodata2[Otodata2$ID!="BH-90910",])+
  geom_point(aes(x = dist, y = OH, color = area), size = 0.05)+
  geom_smooth(aes(x = dist, y = OH, color=area), size=1 , method = "lm")+
  ylim(0.02,0.06)+
  labs(x=expression("distance to core"~(mu*"m")),
       y=expression("OH/"^16*"O count ratio"),
...
d18O <- ggplot(Otodata2[Otodata2$ID!="BH-90910",])+
  geom_point(aes(x = OH, y = d18O, color = area), size = 0.05)+
...
Allplot <- ggarrange(Dist, d18O, common.legend = T)

ggsave("Figures/OH-exploratory-figure.png", Allplot, width = 15, height = 10, units = "cm", dpi = 600, bg = "white")
```
...
barplot(Oto.vif[,3], main = "VIF values", horiz = F, col = "darkgreen", ylim = c(0,6), las = 2)
...
Otos2 <- Otos%>%
  ...
  transmute(ID=ID,
            ...
            sex=case_when(ID == "BH-90307"~"f",
                          TRUE ~ sex),
            area=as.factor(case_when(area == "Sehrowbach"~"Tributary",
...
Otomodel.1 <- lmer(d18O_Oto.scale~OH.scale+sims_dist+inc_um+area+sex+(1|ID), REML = T, data = Otos2)
